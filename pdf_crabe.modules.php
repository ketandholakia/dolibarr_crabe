...
class pdf_crabe extends ModelePDFFactures
{
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			...
			// $pdf->MultiCell(($pdf->GetStringWidth($titre) + 3), 2, $titre); //ketan-disable
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(108, 2, "DESCRIPTION OF GOODS / SERVICES", '', 'C'); //KETAN-DESCRIPTION OF GOOD
		}
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell($this->posxup-$this->posxtva+3, 2, "GST", '', 'C');
			}
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxunit-$this->posxqty-1, 2, "Quantity", '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty-$this->posxup-1, 2, "Unit Price", '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(30, 2, "Taxable Amount", '', 'C');
		}
	}
...
$pdf->Image('https://varniinfoweb.com/erp/core/modules/facture/doc/footer.jpg', 10, 260, 190, 21, 'JPG', 'http://www.varniinfoweb.com', '', true, 150, '', false, false, 0, false, false, false);
		...
		$pdf->Cell(0, 0, "$title", 0, 1, 'C', 0, '', 0); //invoice title KETAN
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
	//	$textref=$outputlangs->transnoentities("Ref")." : " . $outputlangs->convToOutputCharset($object->ref); //ketan-disable
		...
		if ($showaddress)
		{
			...
			// Show recipient
			$widthrecbox=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 45 : 100;
